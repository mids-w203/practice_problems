...
statement: |
    ...
    Consider the fitted values that result from estimating a linear regression without an intercept. That is, suppose you fit the following model: 

    \[
      Y_{i} = 0 + \hat{\beta}_{1}X_{i} + \epsilon_{i}
    \]
