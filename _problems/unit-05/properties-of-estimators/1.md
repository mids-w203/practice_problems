...
$$
\begin{align}
  V[M_{(n)}] &= V[\frac{2}{n} \sum_{i=1}^{n}x_{i}] \\\\
             &= \frac{4}{n^{2}} V[\sum_{i=1}^{n}x_{i}] \\\\
             &= \frac{4}{n^{2}} \sum_{i=1}^{n} V[x_{i}] \\\\
             &= \frac{4}{n^{2}} \sum_{i=1}^{n} V[X] \\\\
\end{align}
$$

At this point, we need to establish $V[X]$. (See how using the CMT made this proof easier. This is an example for why proving pieces and having them in our back pockets can be useful.) Rather than deriving if from whole cloth, we'll note that it is well known that the variance of a uniform distribution of this form is $V[X] = \frac{1}{12}a^{2}$. 

Returning to the statement before: 

$$
\begin{align}
  V[M_{(n)}] &= \frac{4}{n^{2}} \sum_{i=1}^{n} V[X] \\\\
   &= \frac{4}{n^{2}} nV[X] \\\\\
  &= \frac{4}{n} \cdot \frac{1}{12}a^{2}
\end{align}
$$
